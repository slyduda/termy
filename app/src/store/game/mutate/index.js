...
const store = {
    ...
    actions: {
        ...
        submit(context, payload) {
            ...
            if ( !context.getters.validGuess ) {
                ...
                // context.commit('shake', true)
                context.commit('removeLife')
                context.dispatch('save')
                return
            }

            if ( !context.getters.validConstraints ) {
                context.commit('admin/alert', 'Word fails constraints')
            }
            ...
            if ( context.getters.won ) {
                context.dispatch('end', true) // This should save the win
                context.dispatch('admin/alert', "You got it!")
            } else if ( context.getters.availableWords <= 0 ) { 
                context.dispatch('end', false )
                context.dispatch('admin/alert', 'Cannot make any words')
            } else {
                context.commit('addLife')
            }
        ...
        addLetter(context, payload) {
            ...
            if ( context.getters.remainingLetters[payload] <= 0 && payload !== context.getters.nextIsConstraint) return
        ...
        undoGuess (context) {
            if (context.state.guesses.length <= 0 || context.getters.won) return 
            context.commit('undoGuess')
            context.commit('removeUndo')
            context.dispatch('save')
        },
    ...
    getters: {
        ...
        charStatuses: (state, getters) => {
            return Object.fromEntries(ALPHABET.map(k => [k,  getters.nextIsConstraint === k ? 'correct' : getters.remainingLetters[k] === 0 ? 'disabled' :'' ] ))
        },
